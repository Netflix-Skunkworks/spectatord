 ...
 *
 * include/fmt/core.h:2001:35: error: ambiguous overload for ‘operator==’ (operand types are ‘const char* const’ and
 * ‘fmt::v8::basic_string_view<char>’) include/Poco/Dynamic/Var.h:2008:13: note: candidate: ‘bool
 * Poco::Dynamic::operator==(const bool&, const Poco::Dynamic::Var&)’ include/Poco/Dynamic/Var.h:2024:13: note:
 * candidate: ‘bool Poco::Dynamic::operator==(const string&, const Poco::Dynamic::Var&)’
 * include/Poco/Dynamic/Var.h:2056:13: note: candidate: ‘bool Poco::Dynamic::operator==(const char*, const
 * Poco::Dynamic::Var&)’
 */
...
TEST_F(AdminServerTest, GET_metrics)
{
	...
	std::vector<std::string> expected_keys{"age_gauges", "counters",      "dist_summaries",     "gauges",
	                                       "max_gauges", "mono_counters", "mono_counters_uint", "stats",
	                                       "timers"};
	std::vector<std::string> found_keys;
...
TEST_F(AdminServerTest, common_tags_Disallowed_Keys)
{
	...
	res = post("/config/common_tags",
	           R"({"mantisJobId": "foo", "nf.app": "yolo", "mantisJobName": "bar", "mantisUser": ""})");
	expect_bad_request(res);
}
...
{
	HTTPResponse res =
	    post("/config/common_tags", R"({"mantisJobId": "foo", "mantisJobName": "bar", "mantisUser": 1})");
	expect_bad_request(res);
}

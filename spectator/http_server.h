...
class http_server
{
   public:
	...
	void set_read_sleep(std::chrono::milliseconds millis) { read_sleep_ = millis; }

	void set_accept_sleep(std::chrono::milliseconds millis) { accept_sleep_ = millis; }
	...
	class Request
	{
	   public:
		...
		Request(std::string method, std::string path, std::map<std::string, std::string> headers, size_t size,

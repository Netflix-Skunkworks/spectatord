...
{
   public:
	Id(StrRef name, Tags tags) noexcept : name_(name), tags_(std::move(tags)), hash_(0U) {}

	Id(std::string_view name, Tags tags) noexcept : name_(intern_str(name)), tags_(std::move(tags)), hash_(0U) {}

	static auto Of(std::string_view name, Tags tags = {}) -> Id { return Id(name, std::move(tags)); }

	auto operator==(const Id& rhs) const noexcept -> bool { return name_ == rhs.name_ && tags_ == rhs.tags_; }
	...
	[[nodiscard]] auto WithTags(StrRef k1, StrRef v1, StrRef k2, StrRef v2) const -> Id
	{
		// Create a copy
		Tags tags{GetTags()};
		tags.add(k1, v1);
	...
	[[nodiscard]] auto WithStat(StrRef stat) const -> Id { return WithTag(refs().statistic(), stat); };
...
{
	auto operator()(const spectator::Tags& tags) const -> size_t { return tags.hash(); }
};

template <>
struct hash<shared_ptr<spectator::Id>>
{
	auto operator()(const shared_ptr<spectator::Id>& id) const -> size_t { return id->Hash(); }
};

template <>
struct equal_to<shared_ptr<spectator::Id>>
{
	auto operator()(const shared_ptr<spectator::Id>& lhs, const shared_ptr<spectator::Id>& rhs) const -> bool
	{
		return *lhs == *rhs;
	}
};
...
template <>
struct fmt::formatter<spectator::Id> : formatter<std::string_view>
{

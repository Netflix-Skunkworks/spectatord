...
const Timer* find_timer(Registry* registry, const std::string& name, const std::string& status_code)
{
...
{
   public:
	explicit TestRegistry(std::unique_ptr<Config> config) : Registry(std::move(config), spectatord::Logger()) {}
};
...
HttpClientConfig get_cfg(int read_to, int connect_to)
{
	...
	return HttpClientConfig{
	    absl::Milliseconds(connect_to), absl::Milliseconds(read_to), true, true, true, false, true, false, cert_info};
}
...
TEST(HttpTest, Post)
{
	...
	client.Post(url, "Content-type: application/json", post_data.c_str(), post_data.length());
	...
	auto expected_tags = Tags{
	    {"http.method", "POST"},   {"http.status", "200"},    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/foo"},
	    {"ipc.result", "success"}, {"ipc.status", "success"}, {"nf.process", "spectatord"},  {"owner", "spectatord"},
	};
...
TEST(HttpTest, PostUncompressed)
{
	...
	auto url = fmt::format("http://localhost:{}/foo", port);
	const std::string post_data = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
	auto resp = client.Post(url, "Content-type: application/json", post_data.c_str(), post_data.length());
	...
	ASSERT_TRUE(timer_for_req != nullptr);
	auto expected_tags = Tags{
	    {"http.method", "POST"},       {"http.status", "200"},       {"ipc.attempt", "initial"},
	    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/foo"},     {"ipc.result", "success"},
	    {"ipc.status", "success"},     {"nf.process", "spectatord"}, {"owner", "spectatord"},
	};
...
TEST(HttpTest, Timeout)
{
	...
	auto url = fmt::format("http://localhost:{}/foo", port);
	const std::string post_data = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
	auto response = client.Post(url, "Content-type: application/json", post_data.c_str(), post_data.length());
	server.stop();
	...
	auto expected_tags = Tags{
	    {"http.method", "POST"},       {"http.status", "-1"},        {"ipc.attempt", "initial"},
	    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/foo"},     {"ipc.result", "failure"},
	    {"ipc.status", "timeout"},     {"nf.process", "spectatord"}, {"owner", "spectatord"},
	};
...
TEST(HttpTest, Get)
{
	...
	spectator::Tags timer_tags{
	    {"http.method", "GET"},        {"http.status", "200"},       {"ipc.attempt", "initial"},
	    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/get"},     {"ipc.result", "success"},
	    {"ipc.status", "success"},     {"nf.process", "spectatord"}, {"owner", "spectatord"},
	};
...
TEST(HttpTest, Get503)
{
	...
	spectator::Tags err_timer_tags{
	    {"http.method", "GET"},         {"http.status", "503"},       {"ipc.attempt", "initial"},
	    {"ipc.attempt.final", "false"}, {"ipc.endpoint", "/get503"},  {"ipc.result", "failure"},
	    {"ipc.status", "http_error"},   {"nf.process", "spectatord"}, {"owner", "spectatord"},
	};
	spectator::Tags success_timer_tags{
	    {"http.method", "GET"},        {"http.status", "200"},       {"ipc.attempt", "second"},
	    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/get503"},  {"ipc.result", "success"},
	    {"ipc.status", "success"},     {"nf.process", "spectatord"}, {"owner", "spectatord"},
	};
...
void test_method_header(const std::string& method)
{
	...
	spectator::Tags timer_tags{
	    {"http.method", method},       {"http.status", "200"},         {"ipc.attempt", "initial"},
	    {"ipc.attempt.final", "true"}, {"ipc.endpoint", "/getheader"}, {"ipc.result", "success"},
	    {"ipc.status", "success"},     {"nf.process", "spectatord"},   {"owner", "spectatord"},
	};
	auto timer_id = Id::Of("ipc.client.call", std::move(timer_tags));
	auto timer = registry.GetTimer(timer_id);

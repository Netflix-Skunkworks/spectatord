...
auto getMonotonicSampled(std::string_view name) { return spectator::MonotonicSampled(Id::Of(name)); }
...
TEST(MonotonicSampled, Measure)
{
	...
	EXPECT_TRUE(std::isnan(c.SampledRate())) << "MonotonicSamples should reset their value after being measured";
	...
	measures.clear();
	c.Measure(&measures);
	EXPECT_TRUE(measures.empty()) << "MonotonicSampleds should not report delta=0";
}

...
class LogEntry
{
   public:
	...
	{
		id_ = id_.WithTags(intern_str("ipc.attempt"), attempt(attempt_number), intern_str("ipc.attempt.final"),
	...
	{
		id_ =
		    id_.WithTags(intern_str("ipc.result"), intern_str("failure"), intern_str("ipc.status"), intern_str(error));
	}

	void set_success()
	{
		static auto ipc_success = intern_str("success");
		id_ = id_.WithTags(intern_str("ipc.status"), ipc_success, intern_str("ipc.result"), ipc_success);
	}

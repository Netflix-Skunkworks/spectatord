...
TEST(Gzip, SanityCheck)
{
	...
	ASSERT_EQ(gzip_compress(buf, &compressed_size, str_to_compress.c_str(), str_to_compress.length()), Z_OK);

	char uncompressed[64 * 1024];
	auto uncompressed_size = sizeof uncompressed;
	ASSERT_EQ(gzip_uncompress(uncompressed, &uncompressed_size, buf, compressed_size), Z_OK);
	EXPECT_TRUE(memcmp(str_to_compress.c_str(), uncompressed, str_to_compress.length()) == 0);
}

...
class Tags
{
   ...
   public:
	...
	Tags(std::initializer_list<std::pair<std::string_view, std::string_view>> vs) : begin_(&U.entries_[0])
	{
	...
	auto add(const char* k, const char* v) -> void { insert(intern_str(k), intern_str(v)); }

	auto add(std::string_view k, std::string_view v) -> void { insert(intern_str(k), intern_str(v)); }
	...
	[[nodiscard]] auto hash() const -> size_t
	{
		...
		{
			const auto& entry = *it++;
			h += (std::hash<StrRef>()(entry.key) << 1U) ^ std::hash<StrRef>()(entry.value);
		}
		return h;
	}
	...
	{
		const auto* it =
		    std::lower_bound(begin(), end(), key, [](const Tag& tag, const StrRef& k) { return tag.key < k; });
	...
	[[nodiscard]] auto capacity() const -> size_type { return is_large() ? U.capacity_ : kSmallTags; }
   ...
   private:
	...
	{
		auto* it = std::lower_bound(begin(), end(), k, [](const Tag& tag, const StrRef& k) { return tag.key < k; });
...
template <>
struct fmt::formatter<spectator::Tag> : formatter<std::string_view>
{

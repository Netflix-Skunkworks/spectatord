...
auto gzip_compress(void* dest, size_t* destLen, const void* source, size_t sourceLen) -> int
{
...
class inflate_guard
{
   ...
   public:
	explicit inflate_guard(z_stream* stream) : stream_(stream) { init = inflateInit2(stream_, kWindowBits); }
	~inflate_guard()
	{
		if (init == Z_OK)
		{
...
auto gzip_uncompress(void* dest, size_t* destLen, const void* source, size_t sourceLen) -> int
{
	// no initialization due to gcc 4.8 bug
	z_stream stream;
	stream.next_in = reinterpret_cast<z_const Bytef*>(const_cast<void*>(source));

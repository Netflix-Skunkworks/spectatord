...
TEST(PercentileBuckets, IndexOf)
{
	...
	EXPECT_EQ(PercentileBucketsLength() - 1, PercentileBucketIndexOf(std::numeric_limits<int64_t>::max()));
}
...
TEST(PercentileBuckets, Percentiles)
{
	...
	std::vector<double> pcts{0.0, 25.0, 50.0, 75.0, 90.0, 95.0, 98.0, 99.0, 99.5, 100.0};
	std::vector<double> results;
	Percentiles(counts, pcts, &results);
	ASSERT_EQ(results.size(), pcts.size());

	std::vector<double> expected = {0.0, 25e3, 50e3, 75e3, 90e3, 95e3, 98e3, 99e3, 99.5e3, 100e3};

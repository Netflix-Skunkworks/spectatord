...
{
	path_response_["/foo"] = "HTTP/1.0 200 OK\nContent-Length: 3\nX-Test: foo\n\nOK\n";
...
auto http_server::Request::get_header(const std::string& name) const -> std::string
{
...
auto http_server::get_requests() const -> const std::vector<http_server::Request>&
{
...
void http_server::accept_loop()
{
	...
	while (!is_done)
	{
		...
		{
			Logger()->debug("Sleeping before accept {} < {}", slept_number, sleep_number_.load());

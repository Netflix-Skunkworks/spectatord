...
DistributionSummary getDS() { return DistributionSummary(spectator::Id::Of("ds")); }
...
auto expect_dist_summary(const DistributionSummary& ds, int64_t count, int64_t total, double total_sq, int64_t max)
    -> void
{
	using spectator::refs;
	spectator::Measurements ms;
	ds.Measure(&ms);
	ASSERT_EQ(ms.size(), 4) << "Expected 4 measurements from a DistributionSummary, got " << ms.size();

...
#include "spectator/meters/age_gauge.h"
#include "spectator/meters/config.h"
#include "spectator/meters/counter.h"
#include "spectator/meters/dist_summary.h"
#include "spectator/meters/gauge.h"
#include "spectator/meters/max_gauge.h"
#include "spectator/meters/monotonic_counter.h"
#include "spectator/meters/monotonic_counter_uint.h"
#include "spectator/meters/monotonic_sampled.h"
#include "publisher.h"
#include "spectator/meters/timer.h"
...
template <typename T>
inline auto is_meter_expired(int64_t now, const T& m, int64_t meter_ttl) -> bool
{
...
template <>
inline auto is_meter_expired<AgeGauge>(int64_t /* now */, const AgeGauge& /* m */, int64_t /* meter_ttl */) -> bool
{
	return false;
}

template <>
inline auto is_meter_expired<Gauge>(int64_t now, const Gauge& m, int64_t /* meter_ttl */) -> bool
{
...
struct all_meters
{
	...
	{
		return age_gauges_.size() + counters_.size() + dist_sums_.size() + gauges_.size() + max_gauges_.size() +
		       mono_counters_.size() + mono_counters_uint_.size() + timers_.size();
	}
	...
	auto insert_age_gauge(Id id) { return age_gauges_.insert(std::make_shared<AgeGauge>(std::move(id))); }

	auto insert_counter(Id id) { return counters_.insert(std::make_shared<Counter>(std::move(id))); }

	auto insert_dist_sum(Id id) { return dist_sums_.insert(std::make_shared<DistributionSummary>(std::move(id))); }

	auto insert_gauge(Id id, absl::Duration ttl) { return gauges_.insert(std::make_shared<Gauge>(std::move(id), ttl)); }

	auto insert_max_gauge(Id id) { return max_gauges_.insert(std::make_shared<MaxGauge>(std::move(id))); }

	auto insert_mono_counter(Id id) { return mono_counters_.insert(std::make_shared<MonotonicCounter>(std::move(id))); }

	auto insert_mono_counter_uint(Id id)
	{
		return mono_counters_uint_.insert(std::make_shared<MonotonicCounterUint>(std::move(id)));
	}

	auto insert_mono_sampled(Id id) { return mono_sampled_.insert(std::make_shared<MonotonicSampled>(std::move(id))); }

	auto insert_timer(Id id) { return timers_.insert(std::make_shared<Timer>(std::move(id))); }
};
...
class Registry
{
   public:
	...
	using measurements_callback = std::function<void(const std::vector<Measurement>&)>;
	...
	auto GetAgeGauge(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<AgeGauge>;

	auto GetCounter(Id id) noexcept -> std::shared_ptr<Counter>;
	auto GetCounter(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<Counter>;

	auto GetDistributionSummary(Id id) noexcept -> std::shared_ptr<DistributionSummary>;
	auto GetDistributionSummary(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<DistributionSummary>;

	auto GetGauge(Id id) noexcept -> std::shared_ptr<Gauge>;
	auto GetGauge(Id id, absl::Duration ttl) noexcept -> std::shared_ptr<Gauge>;
	auto GetGauge(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<Gauge>;

	auto GetMaxGauge(Id id) noexcept -> std::shared_ptr<MaxGauge>;
	auto GetMaxGauge(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<MaxGauge>;

	auto GetMonotonicCounter(Id id) noexcept -> std::shared_ptr<MonotonicCounter>;
	auto GetMonotonicCounter(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<MonotonicCounter>;

	auto GetMonotonicCounterUint(Id id) noexcept -> std::shared_ptr<MonotonicCounterUint>;
	...
	auto GetMonotonicSampled(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<MonotonicSampled>;

	auto GetTimer(Id id) noexcept -> std::shared_ptr<Timer>;
	auto GetTimer(std::string_view name, Tags tags = {}) noexcept -> std::shared_ptr<Timer>;

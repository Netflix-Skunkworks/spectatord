...
class FileWatcher
{
   public:
	explicit FileWatcher(std::string file_name) noexcept
	    : file_name_(std::move(file_name)), last_updated_(now()), exists_(access(file_name_.c_str(), F_OK) != -1)
	{
	}
	...
	bool exists() const noexcept
	{
		...
		{
			exists_.store(access(file_name_.c_str(), F_OK) != -1, std::memory_order_relaxed);
   ...
   private:
	...
	static int64_t now() { return std::chrono::system_clock::now().time_since_epoch().count(); }

...
class Tags
{
   ...
   public:
	...
	bool operator==(const Tags<C>& that) const { return that.entries_ == entries_; }
...
void add_tags(benchmark::State& state)
{
	...
	for (auto _ : state)
	{
		for (auto i = 0; i < state.range(0); ++i)
		{
			...
			auto long_key = fmt::format("some.longKey.value.that.does.not.fit.in.the.string.{}", i);
...
{
	Tags<T> tags{{"name", "some.name"}, {"id", "some.id"}, {"dev", "root.device"}};
...
static void BM_HopscotchMapHash(benchmark::State& state)
{
	...
	using table_t = tsl::hopscotch_map<std::string, std::string, std::hash<std::string>, std::equal_to<std::string>,
	                                   std::allocator<std::pair<std::string, std::string>>, 4, true>;
	add_tags<table_t>(state);
}

...
std::optional<measurement> get_measurement_ptr(const spectator::Registry* registry, const char* measurement_str,
                                               std::string* err_msg)
{
	// get name
	...
	else
	{
		...
		{
			// just a warning
			*err_msg = fmt::format("Got {} parsing value, ignoring chars starting at {}", value, last_char);
		}
	}
...
std::optional<measurement> get_measurement_strchr(const spectator::Registry* registry, const char* measurement_str,
                                                  std::string* err_msg)
{
...
std::optional<measurement> get_measurement_strview(const spectator::Registry* registry, const char* measurement_str,
                                                   std::string* err_msg)
{
	// get name (tags are specified with , but are optional)
	auto p = std::strpbrk(measurement_str, ",:");
	...
	std::string_view name{measurement_str, static_cast<std::string_view::size_type>(p - measurement_str)};
...
static std::vector<std::string> get_measurements()
{
	...
	measurements.emplace_back("spectatord_test.counter,id=1234,foo=some-foo:42.0\n");
	...
	measurements.emplace_back("spectatord_test.max,id=12341234,foo=some-foo:0.12341234\n");
...
using measurement_fn = std::function<std::optional<measurement>(const spectator::Registry*, const char*, std::string*)>;

static void bench_get_measurement_fun(benchmark::State& state, const measurement_fn& fn)
{
